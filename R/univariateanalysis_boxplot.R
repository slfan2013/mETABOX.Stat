...
univariateanalysis_boxplot <- function(e2,f2,p2,
  ...
  if(is.null(xlab)||is.na(xlab)){
    if(test%in%c("independent*independent")){
  ...
  if(test%in%c("independent*independent")){
    ...
    for(j in index){
      if(f2$missingremoved[j]){
        ...
        data = data.frame(value = e2[j,],group1 = factor(p2[[group1]],levels = factor_order1), group2 = factor(p2[[group2]],levels = factor_order2))
        ...
        boxplot(value~group1*group2,data = data,notch=FALSE,col=terrain.colors(length(factor_order1)),
                xaxt="n",at = at.x)
        title(main=f2[["Binbase name"]][j], col.main=col_main,
              xlab=xlab, ylab=ylab,
              col.lab=col_lab)
    ...
    data = data.frame(value = e2[j,],group1 = factor(p2[[group1]],levels = factor_order1))
    png(paste0(j,'th_',f2[j,"BinBase name"],'.png'), width = 800, height = 600)
